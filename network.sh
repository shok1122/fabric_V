...
{
    local dummy
    read TIME dummy < /proc/uptime
    TIME_LOG="${TIME_LOG}${TIME},$1\n"
}
...
function networkUp() {
  ...
  set_time "start_UpContainer"
  IMAGE_TAG=$IMAGETAG docker-compose ${COMPOSE_FILES} up -d 2>&1
  set_time "finish_UpContainer"
  docker ps -a
  if [ $? -ne 0 ]; then
...
function generateCerts() {
  ...
  set -x
  cryptogen generate --config=./crypto-config-1.yaml
  res=$?
  set +x
  if [ $res -ne 0 ]; then
...
function generateChannelArtifacts() {
  ...
  set_time "start_generateChannelArtifacts"
  ...
  echo "# ================================================================"
  set_time "start_generateGenesisBlock"
  ...
  echo
  set_time "finish_generateGenesisBlock"
  echo "# ================================================================"
  echo "# Generating channel configuration transaction 'channel.tx'"
  echo "# ================================================================"
  set_time "start_generateChannelConfig"
  set -x
  ...
  for list in $(yq .Profiles.OrgsOrdererGenesis.Consortiums.SampleConsortium.Organizations[].ID configtx.yaml); do
      ...
      set_time "finish_generateAnchorPeer_$org"
  done

  echo

  set_time "finish_generateChannelArtifacts"
}
...
COMPOSE_FILE=docker-compose-cli-1.yaml
#
...
echo -e $TIME_LOG > "log/network.${MODE}.log"
